...
if(isset($_POST["submit"])){
	...
	$shippingPostcode = mysqli_real_escape_string($con, $_POST["shipping_postcode"]);
    ...
    if($con->query($sqlContact)){
		...
		SHIPPING_ADDRESS_LINE_1, SHIPPING_ADDRESS_LINE_2, COUNTRY_REGION, STATE, CITY, POSTCODE, REMARK, PICKUP_DATE, DELIVERY_TIME) 
		VALUES ('{$uid}', '{$shippingFirstName}', '{$shippingLastName}', '{$shippingEmail}', '{$shippingPhone}', '{$shippingCompany}', 
		'{$shippingAddress1}', '{$shippingAddress2}', '{$shippingCountry}', '{$shippingState}', '{$shippingCity}', '{$shippingPostcode}', '{$remark}', '{$pickupDate}', '{$deliveryTime}')";
        ...
        if($con->query($sqlShipping)){
			...
			if ($con->query($sqlOrder)) {
				...
				// Store order number and total amount in session for later use
				$_SESSION['order_no'] = $order_no; // Order number
				$_SESSION['total_amt'] = $total_amt; // Total amount
				$_SESSION['delivery_date'] = $pickupDate;
				...
				if ($con->query($sqlOrderDetails)) {
					$cart->destroy();
					header("location:complete_delivery.php"); // Redirect to complete.php
				}
			}
		...
		<script>
            function checkPickupDate(input) {
                ...
                if (selectedDate.toDateString() === currentDate.toDateString() && hours >= 16) {
                    alert("Please select a date after today its after 4PM for today's date we unable to deliver your parcel by today:) thanks.");
